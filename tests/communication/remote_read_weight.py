...
arch_configs = config_analyzer(os.path.join(project_root, "arch", "myarch_gemini4_4.json"))
...
if __name__ == "__main__":
    ...
    for core_y in range(arch_configs.core.y):
        ...
        # 假设是4*4的mesh,第二列core
        cur_start_m = 1*tile_size_m
        cur_size_m = min(tile_size_m, total_M - cur_start_m)
        weight_range_left = Slice(tensor_slice=[
        ...
        list_core_id = get_list_id(1, core_y)
...
wl = Workload(name="remote_read_weight", pes=pewls)
workload_json = wl.model_dump_json(indent=4)

output_file_path = os.path.join(project_root, "tests", "communication", "remote_read_weight.json")
