                    ...
                    if arch_configs.mem.type == "two_sides_edge": 
                        ...
                        for core_y in range(arch_configs.core.y):
                            ...
                            for broadcast_index in range(1,arch_configs.core.x):
                                info = (list_core_id, get_list_id(broadcast_index, core_y), lid, "activation", act_range_left.tensor_slice[0].start, act_range_left.tensor_slice[1].start,act_range_left.tensor_slice[2].start,act_range_left.tensor_slice[3].start)
                            ...
                            for broadcast_index in range(0,arch_configs.core.x-1):
                                info = (list_core_id, get_list_id(broadcast_index, core_y), lid, "activation", act_range_right.tensor_slice[0].start, act_range_right.tensor_slice[1].start,act_range_right.tensor_slice[2].start,act_range_right.tensor_slice[3].start)
                                send_map[info] = global_inst_id
                            ...
                            for core_x in range(0, arch_configs.core.x):
                                ...
                                if core_x != 0: # 不是每行最左边的核，需要RECV被广播的act_left
                                    info = (get_list_id(0, core_y), list_core_id, lid, "activation", act_range_left.tensor_slice[0].start, act_range_left.tensor_slice[1].start,act_range_left.tensor_slice[2].start,act_range_left.tensor_slice[3].start) 
                                    pewls[list_core_id].insts.append(
                                            Instruction(
                                ...
                                if core_x != arch_configs.core.x-1:
                                    info = (get_list_id(arch_configs.core.x-1, core_y), list_core_id, lid, "activation", act_range_right.tensor_slice[0].start, act_range_right.tensor_slice[1].start,act_range_right.tensor_slice[2].start,act_range_right.tensor_slice[3].start) 
                                    pewls[list_core_id].insts.append(
                                            Instruction(
                                                inst_type = TaskType.RECV,
                                                index = send_map[info],
