...
def cmd_stream_gen(input_model: TransformerModel, n_micro_batch: int, tp_config: TPConfig, single_tp: bool = False):
    tp_networks = build_tp_networks(input_model, tp_config)
    for idx, core_id in enumerate(tp_config.core_list):
        if single_tp and idx > 0:
            continue
...
def cmd_stream_gen_pptp(input_model: TransformerModel, n_micro_batch: int, pptp_config: PPTPConfig, single_tp: bool = False):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-il", "--input_len", type=int, default=None, help="override input_len from config")
    parser.add_argument("-stp", "--single_tp", action="store_true", help="only generate instructions for one core per TP group")
    ...
    input_len = input_len_override if input_len_override is not None else model["input_len"]
    ...
    if use_pptp:
        ...
        cmd_stream_gen_pptp(transformer_model, n_micro_batch, pptp_config, single_tp=args.single_tp)
    else:
        ...
        cmd_stream_gen(transformer_model, n_micro_batch, tp_config, single_tp=args.single_tp)
