...
def layer_cmd_gen_for_single_core( network: Transformer_Network, core_id: int, tf_layer_id: int, next_core_id: int, n_micro_batch: int, pipe_recv_en: False, pipe_sent_en: False, finish: False):
    ...
    for loop_id in range(pipeline_config.loop):            
        for i, core_id in enumerate(pipeline_config.core_list):
            for layer_id in range(layer_per_loop_stage):
                ...
                layer_cmd_gen_for_single_core(tf_network, core_id, tf_layer_id, next_core_id, n_micro_batch, pipe_recv_en, pipe_sent_en, finish)
...
if __name__ == "__main__":
    ...
    batch = 64
    micro_batch = 1
    n_micro_batch = batch // micro_batch
