...
arch_configs = config_analyzer("arch\cim.json")
...
def GEMM(read_activation=True, read_weight=True, write_back=True):
    ...
    send_map = {}
    bubble=arch_configs.core.x-tile_num_k%arch_configs.core.x
    ...
    for n in range(tile_num_n):
        ...
        for tp_m in range(temporal_loop_m): # M-X维度时域循环
            for tp_k in range(temporal_loop_k): # K-Y维度时域循环
                is_last=(tp_m==(temporal_loop_m-1))
                #((temporal_loop_m-1)*(temporal_loop_k-1)-tp_k*tp_m)==0
                ...
                if read_activation:
                    ...
                    if arch_configs.mem.type == "two_sides_edge": 
                        ...
                        for core_y in range(arch_configs.core.y):
                            ...
                            if (tp_k,tp_m) not in act_providers_map[list_core_id]: act_providers_map[list_core_id][(tp_k,tp_m)] = []
                            act_providers_map[list_core_id][(tp_k,tp_m)].append(read_act_inst)
                            ...
                            send_inst = Instruction(
                                ...
                                data_type = DataType.FEAT,
                                feat_num = 1,
                                position = get_list_id(arch_configs.core.x-1, core_y),  # 给出send的终点
                                tensor_slice = act_range_left.tensor_slice
                            )
                            ...
                            for broadcast_index in range(1,arch_configs.core.x):   # 1,2,3,4都被广播到
                                ...
                                if (tp_k,tp_m) not in act_providers_map[dst_core]: act_providers_map[dst_core][(tp_k,tp_m)] = []
                                act_providers_map[dst_core][(tp_k,tp_m)].append(recv_inst)
                ...
                if read_weight:
                    # 读入weight
                    for core_x in range(arch_configs.core.x-bubble*is_last):
                        for core_y in range(arch_configs.core.y):
            ...
            for core_x in range(arch_configs.core.x-bubble*is_last):
                ...
                for sender_y in range(root_y):
                    ...
                    recv_inst.trigger_index.append(elem_inst.index)  # 新增：RECV触发ELEM
                    global_inst_id += 1
        ...
        for core_id, comp_map_for_core in comp_inst_map.items():
            for (tp_k, tp_m), comp_inst in comp_map_for_core.items():
                ...
                if core_id in act_providers_map and (tp_k,tp_m) in act_providers_map[core_id]:
                    act_insts = act_providers_map[core_id][(tp_k,tp_m)]
...
if __name__ == "__main__":
    ...
    output_file = "tests/gemm/workload_gemm_test.json"
    ...
    print(f"测试workload已保存至: {output_file}")